...
# Country distribution: mostly Egypt (domestic), occasionally foreign for fraud sim
DOMESTIC_COUNTRIES = ["Egypt"]
FOREIGN_COUNTRIES = ["Russia", "Nigeria", "China", "Brazil", "Germany", "UK", "USA"]


def generate_transaction():
    is_fraud = random.random() < 0.03

    # Fraud transactions more likely to originate from foreign countries
    if is_fraud:
        country = random.choice(FOREIGN_COUNTRIES) if random.random() < 0.7 else "Egypt"
    else:
        country = "Egypt" if random.random() < 0.95 else random.choice(FOREIGN_COUNTRIES)
...
def run_producer():
    print("🚀 Producer started (with country field for MCP integration)...")
    count = 0
